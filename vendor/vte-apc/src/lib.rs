...
impl Parser {
        ...
        match state {
            ...
            state => {
                match self.state {
                    ...
                    // zellij-vte-apc fork: leaving APC flushes the buffered
                    // payload. `self.state` is still the OLD state here (it is
                    // assigned below), so this correctly fires only on exit.
                ...
                match state {
                    ...
                    // zellij-vte-apc fork: entering APC clears the reused
                    // `osc_raw` buffer, mirroring `Action::OscStart`.
                    State::SosPmApcString => {
                        self.osc_raw.clear();
                    },
                    _ => (),
    ...
    /// zellij-vte-apc fork: dispatch the accumulated APC payload.
    ///
    /// Borrows self read-only. APC has a single raw payload (no `;`-split
    /// params), so no `MaybeUninit` aliasing dance is needed (unlike
    /// `osc_dispatch`).
    #[inline]
    ...
    fn perform_action<P: Perform>(&mut self, performer: &mut P, action: Action, byte: u8) {
        match action {
            ...
            // zellij-vte-apc fork: in the APC state, buffer the body into the
            // reused `osc_raw` buffer instead of streaming via `put` (which is
            // for DCS). `self.state` is the OLD state here (assigned only after
            // the action runs), so it correctly distinguishes APC from DCS.
            Action::Put => {
...
pub trait Perform {
    ...
    /// and terminator (`ESC \` / C1 `0x9c`) already stripped. The bytes are
    /// buffered and delivered once on the terminating ST. The default empty body
    /// keeps every existing `Perform` impl compiling unchanged.
    ///
    /// NOTE (zellij-vte-apc fork): SOS (`ESC X`) and PM (`ESC ^`) share vte's
    /// internal `SosPmApcString` state, so their payloads are also delivered
    /// here. Consumers that only want APC must filter by content.
...
mod tests {
    ...
    // ---- zellij-vte-apc fork: APC dispatch tests ----

    fn feed(dispatcher: &mut Dispatcher, parser: &mut Parser, bytes: &[u8]) {
        for byte in bytes {
            parser.advance(dispatcher, *byte);
        }
    }

    fn only_apc(dispatcher: &Dispatcher) -> &[u8] {
        let apcs: Vec<&Vec<u8>> = dispatcher
            .dispatched
            .iter()
            .filter_map(|s| match s {
                Sequence::Apc(b) => Some(b),
                _ => None,
            })
            .collect();
        assert_eq!(apcs.len(), 1, "expected exactly one APC dispatch: {:?}", dispatcher.dispatched);
        apcs[0]
    }

    #[test]
    fn apc_c1_st_terminator() {
        // ESC _ G f o o  <0x9c ST>
        let mut d = Dispatcher::default();
        let mut p = Parser::new();
        feed(&mut d, &mut p, &[0x1b, 0x5f, b'G', b'f', b'o', b'o', 0x9c]);
        assert_eq!(only_apc(&d), b"Gfoo");
    }

    #[test]
    fn apc_esc_backslash_terminator() {
        // ESC _ G f o o  ESC \
        let mut d = Dispatcher::default();
        let mut p = Parser::new();
        feed(&mut d, &mut p, &[0x1b, 0x5f, b'G', b'f', b'o', b'o', 0x1b, 0x5c]);
        assert_eq!(only_apc(&d), b"Gfoo");
    }

    #[test]
    fn apc_split_across_feeds() {
        // payload split mid-body across two advance batches; only ST flushes
        let mut d = Dispatcher::default();
        let mut p = Parser::new();
        feed(&mut d, &mut p, &[0x1b, 0x5f, b'G', b'f']);
        assert!(d.dispatched.iter().all(|s| !matches!(s, Sequence::Apc(_))), "must not flush early");
        feed(&mut d, &mut p, &[b'o', b'o', 0x9c]);
        assert_eq!(only_apc(&d), b"Gfoo");
    }

    #[test]
    fn apc_buffer_cleared_between_sequences() {
        let mut d = Dispatcher::default();
        let mut p = Parser::new();
        feed(&mut d, &mut p, &[0x1b, 0x5f, b'G', b'a', b'a', 0x9c]);
        feed(&mut d, &mut p, &[0x1b, 0x5f, b'G', b'b', 0x9c]);
        let apcs: Vec<&Vec<u8>> = d
            .dispatched
            .iter()
            .filter_map(|s| match s {
                Sequence::Apc(b) => Some(b),
            ...
            .collect();
        assert_eq!(apcs.len(), 2);
        assert_eq!(apcs[0], b"Gaa");
        assert_eq!(apcs[1], b"Gb"); // second must not contain first's bytes
    }

    #[test]
    fn apc_delivers_non_g_payloads_verbatim() {
        // The fork is content-agnostic; the `G` filter is the consumer's job.
        // SOS (ESC X) shares the state and is also delivered here.
        let mut d = Dispatcher::default();
        let mut p = Parser::new();
        feed(&mut d, &mut p, &[0x1b, 0x58, b'X', b'b', b'a', b'r', 0x9c]); // ESC X = SOS
        assert_eq!(only_apc(&d), b"Xbar");
    }

    #[test]
    fn apc_does_not_break_dcs_put() {
        // A DCS sequence must still stream byte-by-byte via put(), proving the
        // state-aware Action::Put did not divert DCS into the APC buffer.
        let mut d = Dispatcher::default();
        let mut p = Parser::new();
        // ESC P (DCS) q <final, triggers hook> Z <passthrough put byte> ESC \
        feed(&mut d, &mut p, &[0x1b, 0x50, b'q', b'Z', 0x1b, 0x5c]);
        assert!(d.dispatched.iter().any(|s| matches!(s, Sequence::DcsHook(..))));
        assert!(d.dispatched.iter().any(|s| matches!(s, Sequence::DcsPut(b'Z'))));
        assert!(d.dispatched.iter().any(|s| matches!(s, Sequence::DcsUnhook)));
        assert!(d.dispatched.iter().all(|s| !matches!(s, Sequence::Apc(_))));
    }

    #[test]
    fn lone_c1_apc_introducer_is_dropped() {
        // 0x9f (C1 APC introducer) is intentionally NOT wired in Phase 0;
        // documents that only ESC _ enters the APC buffer.
        let mut d = Dispatcher::default();
        let mut p = Parser::new();
        feed(&mut d, &mut p, &[0x9f, b'G', b'x', 0x9c]);
        assert!(d.dispatched.iter().all(|s| !matches!(s, Sequence::Apc(_))));
    }
